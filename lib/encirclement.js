...
function fillEnclosures(fillList, mapTile, playerColor) {
  ...
  while(fillList.length > 0) {
    ...
    if (mapTile.colors[x][y].mark == undefined || mapTile.colors[x][y].mark == false) {
      ...
      fillCells.push([x,y]);
	  worldmap.writeMapTile(mapTile, {x:x, y:y}, playerColor);
...
function encirclement(placedX, placedY, playerColor) {
  ...
  var xy = worldmap.tileXYFromGXY(placedX,placedY);
	...
	if (mapTile.colors[xy.x][xy.y].color != playerColor) {
  ...
  // TODO HACK!!!
  // following resetting of placedX,placedY is jsut to get it to work within one tile
  // need to fix!
  placedX = xy.x;
  placedY = xy.y;
	var fillList = [];
